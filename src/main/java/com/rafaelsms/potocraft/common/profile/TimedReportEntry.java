...
public class TimedReportEntry extends ReportEntry {
    ...
    public TimedReportEntry(@NotNull ReportType reportType, @Nullable UUID reporterId,
                            @Nullable ZonedDateTime expirationDate, @Nullable String reason) {
        super(reportType, reporterId, reason);
        if (reportType == ReportType.KICK_REPORT) throw new IllegalArgumentException("Kick reports are not timed");
